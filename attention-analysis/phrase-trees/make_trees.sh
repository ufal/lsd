...
#for i in sent-per-file/s[1234]?.txt; do
    ...
    ./attentions2tree_sent-per-file.py --attentions $DATA_DIR/sent-per-file-out/s$sentnum.att.npz --labels $DATA_DIR/sent-per-file/s$sentnum.wps --alignment $DATA_DIR/sent-per-file/s$sentnum.ali --heatmaps $DATA_DIR/sent-per-file-out/s$sentnum --tree $DATA_DIR/sent-per-file-out/s$sentnum.tree
