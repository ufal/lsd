...
TRAINCONLLU=/net/projects/LSD/attention_tomasz/lsd/attention-analysis/naacl2019/graph-extraction/${LAN}train.conllu
...
do
  b=$(basename $f)
  if [ ! -d  "$OUTPUT_DIR/$b/multihead+pos+hard" ]
  then
    mkdir -p "$OUTPUT_DIR/$b/multihead+pos+hard"
  fi
  python $UAS_SCRIPT -a "$f/attentions.npz" -t "$f/source.txt" -u "$OUTPUT_DIR/$b/multihead+pos+hard/mhuas" -c $CONVCONLLU -e -T $CONVTRAINCONLLU
done
